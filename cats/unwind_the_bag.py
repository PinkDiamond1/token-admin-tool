...
async def unwind_coin_spend(full_node_client: FullNodeRpcClient, tail_hash_bytes: bytes32, coin_spend: CoinSpend) -> SpendBundle:
    ...
    # Get parent coin info as required for lineage proof when spending this token coin
    ...
    if parent_matched is None:
        raise Exception("Expected parent to be token")
            ...
            for coin_spend in level.values():
                ...
                if len(bundle_spends) >= batch_size or i == len(level.values()):
                    if unwind_fee > 0:
                        ...
                        # Fees depend on announcements made by secure the bag tokens to ensure they can't be seperated
                        cat_announcements: List[Announcement] = []
...
@click.option(
    ...
    "--tail-hash",
    required=True,
    help="TAIL hash / Asset ID of token to unwind from secured bag of tokens",
)
@click.option(
    "-stbtp",
...
def cli(
    ...
    chia_root: Path = Path(os.path.expanduser(os.getenv("BPX_ROOT", "~/.bpx/mainnet"))).resolve()
